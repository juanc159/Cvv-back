...
Route::get('/limpiar-boletines', function () {
    // 1. Buscamos solo los estudiantes que tienen algo en la columna boletin
    // Usamos cursor() para manejar grandes cantidades de datos sin saturar la memoria
    $students = Student::whereNotNull('boletin')->where('boletin', '!=', '')->cursor();

    $filesDeleted = 0;
    $dbUpdated = 0;

    foreach ($students as $student) {
        // La ruta que tienes en la BD: company_1/student/student_1/IWnbNDq...pdf
        $path = $student->boletin;

        // 2. Eliminamos el archivo físico
        // IMPORTANTE: Asegúrate de usar el disco correcto (public, local, s3, etc.)
        // Si usas el disco por defecto (usualmente 'public' o 'local'):
        if (Storage::exists($path)) {
            Storage::delete($path);
            $filesDeleted++;
        } 
        // Si usas un disco específico (ej: 'public'), usa: Storage::disk('public')->delete($path);
    }

    // 3. Actualizamos la base de datos masivamente (es más rápido que uno por uno)
    $dbUpdated = Student::whereNotNull('boletin')
        ->where('boletin', '!=', '')
        ->update(['boletin' => null]);
